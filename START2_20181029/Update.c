...
{
    struct imageHeader *Image = (struct imageHeader *)buffer;
    if (Image->magic != MAGIC_NUMBER) {
        PR("Error In Load Image Header, Magic Number Is Wrong.\n");
        return 0;
    }
    if (length - Image->length > 62) {
        PR("Error In Load Image Header, Length Is Wrong.\n");
        return 0;
    }
    if (Image->check != GetCRC32CheckSum(buffer + sizeof(struct imageHeader), Image->length - sizeof(struct imageHeader))) {
        PR("Error In Load Image Header, Check Is Wrong.\n");
        return 0;
    }
...
int UpdateWriteOneSection(unsigned char *buffer, unsigned int length, unsigned int mark)
{
    ...
    switch (mark) {
        case UPDATE_MARK_SHELL:
            sprintf(fileName, "echo2printk.sh");
            break;
        ...
        case UPDATE_MARK_VIDEO_MODULE:
            ...
            break;
        case UPDATE_MARK_SPI_MODULE:
            sprintf(fileName, "SPISlaveDevice.ko");
            break;
        case UPDATE_MARK_LED_MODULE:
            sprintf(fileName, "LED.ko");
            break;
        case UPDATE_MARK_MAIN_EXE:
            sprintf(fileName, "start2_20181029");
            break;
        case UPDATE_MARK_9:
            sprintf(fileName, "picture3.jpg");
            break;
        case UPDATE_MARK_10:
            sprintf(fileName, "picture4.jpg");
            break;
        default:
            PR("Error in Write One Section, Mark Error.\n");
            return -1;
    }
    ...
    if (ret == 1)
        ...
        PR("Error in Write One Section, Write Error.\n");
        return -2;
    }
}
...
void UpdateDecompress(unsigned char *buffer, unsigned int length)
{
    ...
    unsigned char *aim = NULL;
    struct sectionHeader *Section = NULL;
    sectionCount = UpdateCheckImageHeader(buffer, length);
    if (sectionCount == 0) {
        PR("Error In Update Process, Check Failed!\n");
        return;
    }
